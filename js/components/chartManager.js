export class ChartManager {
    ...
    // Hiển thị snapshot (single-point) trong một khoảng thời gian, sau đó khôi phục dữ liệu realtime
    displaySnapshot(snapshot, duration = 5000) {
        if (!snapshot || !snapshot.data) return;

        // Lưu trạng thái hiện tại (clone sâu)
        this._savedState = JSON.parse(JSON.stringify(this.chart.data));

        // Thay bằng dữ liệu snapshot (một điểm duy nhất)
        const label = snapshot.time || new Date().toLocaleTimeString();
        this.chart.data.labels = [label];
        const d = snapshot.data;
        this.chart.data.datasets[0].data = [d.temp];
        this.chart.data.datasets[1].data = [d.humidity];
        this.chart.data.datasets[2].data = [d.gas];
        this.chart.data.datasets[3].data = [d.dust];
        this.chart.data.datasets[4].data = [d.iaq];
        this.chart.update();

        // Tự động khôi phục sau duration
    ...
    restoreState() {
        ...
        this.chart.data = this._savedState;
        this._savedState = null;
        if (this._restoreTimer) {
            clearTimeout(this._restoreTimer);
